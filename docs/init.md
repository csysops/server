...
## Socket
- is abstraction, an illusion communicating through network same as operating on file, `send/receive` == `write/read`, include src[`ip`, `port`] && des[`ip`, `port`], one `socket` -> one `port`,
